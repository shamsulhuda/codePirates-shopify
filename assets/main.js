...
(function ($) {
	...
	/*-----------------------------------------
	  10.6 Product Slider Slick Product Page --
	  ----------------------------------------- */
  function product_slider_recomm(){
		$('.productPageRecomm').slick({
			dots: false,
			infinite: true,
			slidesToShow: 5,
			slidesToScroll: 1,
   ...
   window.addEventListener("DOMContentLoaded",()=>{
    let templateName = $('.related-product').attr('data-template');
    if(templateName == 'product'){
      let myInterval = setInterval(function(){
        let sectionFinder = $(document).find('.productPageRecomm');
        if(sectionFinder.length > 0 ){
          product_slider_recomm();
